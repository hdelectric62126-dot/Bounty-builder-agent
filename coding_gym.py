...
EXERCISES = (
    ...
    Exercise(
        "constant-time-token-check", "authentication_security", 6,
        "import hmac\ndef authorized(expected,supplied): return expected == supplied\n",
        "import hmac\ndef authorized(expected,supplied):\n if not isinstance(expected,str) or not isinstance(supplied,str) or not expected: return False\n return hmac.compare_digest(expected.encode(),supplied.encode())\n",
        "from unittest import TestCase\nfrom solution import authorized\nclass T(TestCase):\n def test_match(self): self.assertTrue(authorized('secret','secret'))\n def test_empty(self): self.assertFalse(authorized('',''))\n def test_types(self): self.assertFalse(authorized('secret',None))\n",
        "Authentication checks must fail closed and use constant-time comparison.",
    ),
    Exercise(
        "legal-state-transition", "state_management", 5,
        "def transition(current,target): return target\n",
        "ALLOWED={'NEW':{'QUOTED'},'QUOTED':{'PAID','CANCELLED'},'PAID':{'IN_PROGRESS'},'IN_PROGRESS':{'REVIEW'}}\ndef transition(current,target):\n if target not in ALLOWED.get(current,set()): raise ValueError('illegal transition')\n return target\n",
        "from unittest import TestCase\nfrom solution import transition\nclass T(TestCase):\n def test_forward(self): self.assertEqual('QUOTED',transition('NEW','QUOTED'))\n def test_skip(self):\n  with self.assertRaises(ValueError): transition('NEW','PAID')\n def test_reverse(self):\n  with self.assertRaises(ValueError): transition('PAID','NEW')\n",
        "Model explicit workflow transitions so callers cannot skip approval stages.",
    ),
    Exercise(
        "structured-audit-event", "observability", 4,
        "def event(kind,job_id,details): return {'message':str(details)}\n",
        "def event(kind,job_id,details):\n if not kind or not job_id: raise ValueError('identity')\n safe={k:v for k,v in details.items() if k not in {'token','password','secret'}}\n return {'event':kind,'job_id':job_id,'details':safe}\n",
        "from unittest import TestCase\nfrom solution import event\nclass T(TestCase):\n def test_shape(self): self.assertEqual('done',event('done','j1',{})['event'])\n def test_redacts(self): self.assertNotIn('token',event('done','j1',{'token':'x','count':2})['details'])\n def test_identity(self):\n  with self.assertRaises(ValueError): event('','j1',{})\n",
        "Emit structured, attributable audit evidence without leaking credentials.",
    ),
    Exercise(
        "migration-plan", "data_migrations", 6,
        "def migrate(rows): return [{'id':r['id'],'status':'active'} for r in rows]\n",
        "def migrate(rows):\n seen=set(); output=[]\n for row in rows:\n  if row['id'] in seen: raise ValueError('duplicate id')\n  seen.add(row['id']); copy=dict(row); copy.setdefault('status','active'); output.append(copy)\n return output\n",
        "from unittest import TestCase\nfrom solution import migrate\nclass T(TestCase):\n def test_preserves(self): self.assertEqual('Ada',migrate([{'id':1,'name':'Ada'}])[0]['name'])\n def test_idempotent(self): self.assertEqual('paused',migrate([{'id':1,'status':'paused'}])[0]['status'])\n def test_duplicate(self):\n  with self.assertRaises(ValueError): migrate([{'id':1},{'id':1}])\n",
        "Migrations must preserve data, be idempotent, and reject ambiguous identities.",
    ),
    Exercise(
        "explicit-error-envelope", "error_handling", 4,
        "def run(action): return {'ok':True,'value':action()}\n",
        "def run(action):\n try: return {'ok':True,'value':action(),'error':None}\n except (ValueError,RuntimeError) as exc: return {'ok':False,'value':None,'error':type(exc).__name__}\n",
        "from unittest import TestCase\nfrom solution import run\nclass T(TestCase):\n def test_value(self): self.assertEqual(3,run(lambda:3)['value'])\n def test_expected(self): self.assertEqual('ValueError',run(lambda:(_ for _ in ()).throw(ValueError()))['error'])\n def test_does_not_hide_programmer_error(self):\n  with self.assertRaises(TypeError): run(lambda:1+'x')\n",
        "Return bounded expected failures while allowing programmer errors to remain visible.",
    ),
    Exercise(
        "node-strict-config", "input_validation", 4,
        "export function parse(raw) { return JSON.parse(raw); }\n",
        "export function parse(raw) { const v=JSON.parse(raw); if(!v || Array.isArray(v) || typeof v!=='object') throw new TypeError('shape'); if(Object.keys(v).sort().join(',')!=='enabled,retries' || typeof v.enabled!=='boolean' || !Number.isInteger(v.retries) || v.retries<0 || v.retries>5) throw new TypeError('contract'); return v; }\n",
        "import test from 'node:test'; import assert from 'node:assert/strict'; import {parse} from './solution.mjs';\ntest('valid',()=>assert.deepEqual(parse('{\"enabled\":true,\"retries\":2}'),{enabled:true,retries:2}));\ntest('extra',()=>assert.throws(()=>parse('{\"enabled\":true,\"retries\":2,\"secret\":\"x\"}'),TypeError));\n",
        "Validate exact JavaScript object contracts before accepting external input.", "node",
    ),
    Exercise(
        "node-stable-deduplication", "algorithm", 3,
        "export function unique(values) { return [...new Set(values)].sort(); }\n",
        "export function unique(values) { return [...new Set(values)]; }\n",
        "import test from 'node:test'; import assert from 'node:assert/strict'; import {unique} from './solution.mjs';\ntest('stable',()=>assert.deepEqual(unique([3,1,3,2,1]),[3,1,2]));\n",
        "Preserve priority order when deduplicating JavaScript collections.", "node",
    ),
    Exercise(
        "node-bounded-retry", "job_reliability", 5,
        "export async function retry(action) { return action(); }\n",
        "export async function retry(action,attempts=3) { if(!Number.isInteger(attempts)||attempts<1||attempts>5) throw new RangeError('attempts'); let last; for(let i=0;i<attempts;i++){try{return await action();}catch(e){last=e;}} throw last; }\n",
        "import test from 'node:test'; import assert from 'node:assert/strict'; import {retry} from './solution.mjs';\ntest('retries',async()=>{let n=0; assert.equal(await retry(async()=>{if(++n<3)throw Error('x');return 'ok';}), 'ok'); assert.equal(n,3);});\ntest('bounded',async()=>await assert.rejects(retry(async()=>{},0),RangeError));\n",
        "Async retries must be bounded and return the final verified result.", "node",
    ),
    Exercise(
        "node-safe-state-transition", "state_management", 6,
        "export function transition(current,target) { return target; }\n",
        "const allowed={NEW:['QUOTED'],QUOTED:['PAID','CANCELLED'],PAID:['IN_PROGRESS'],IN_PROGRESS:['REVIEW']}; export function transition(current,target){if(!(allowed[current]||[]).includes(target))throw new RangeError('illegal transition');return target;}\n",
